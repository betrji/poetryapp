...
import type { ImageCaptureProps } from '../types';

function ImageCapture({ onImageCapture, initialImage = null }: ImageCaptureProps) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('image/')) {
      if (file.size > 1024 * 1024) {
        ...
        reader.onload = (event) => {
          if (onImageCapture) onImageCapture(event.target?.result as string);
        };
  ...
  function compressAndConvertToBase64(file: File, callback: (base64: string) => void) {
