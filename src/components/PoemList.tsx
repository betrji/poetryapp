...
import type { Poem, PoemListProps } from '../types';

function PoemList({ poems, handleDelete, handleReorder }: PoemListProps) {
  const [draggedIndex, setDraggedIndex] = useState<number | null>(null);
  const [currentOrder, setCurrentOrder] = useState<Poem[]>(poems);
        ...
        {currentOrder.length > 0 ? (
          <div className="grid gap-y-24 gap-x-16 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-items-center mb-16">
            {currentOrder.map((poem, index) => (
              <div 
                key={poem.id ? `poem-${poem.id}` : `poem-index-${index}`}
